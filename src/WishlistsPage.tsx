...
export default function WishlistsPage({ownerId,cards}:{ownerId:string;cards:Card[]}){
 ...
 const [exchange,setExchange]=useState<{card:Card;contact:string}|null>(null)
 ...
 {tab==='contacts'?<div className="owners-panel"><div className="owners-summary"><UsersRound/><div><h3>{owners.length} contact{owners.length>1?'s':''} ont au moins une carte</h3><p>{selectedCards.length-missing.length} carte{selectedCards.length-missing.length>1?'s':''} trouvée{selectedCards.length-missing.length>1?'s':''} dans votre réseau.</p></div><button onClick={()=>copy(contactText,'Texte copié.')}><Clipboard size={17}/>Copier le texte</button></div><div className="owner-groups">{owners.map(([name,names])=><article key={name} onClick={()=>{const card=selectedCards.find(item=>item.contacts.includes(name));if(card)setExchange({card,contact:name})}}><i>{name[0]}</i><div><h3>{name}</h3><p>{names.join(' · ')}</p></div><b>{names.length}</b></article>)}</div><label>Texte prêt à envoyer<textarea value={contactText} readOnly/></label></div>:null}</section>:<div className="wishlist-no-list"><Heart/><h2>Créez votre première liste</h2><button onClick={addList}>Nouvelle liste</button></div>}</div>{notice?<div className="toast"><Check size={17}/>{notice}</div>:null}{exchange?<ExchangeModal card={exchange.card} contact={exchange.contact} onClose={()=>setExchange(null)}/>:null}</div>
}

function WishlistCard({card,onRemove,onExchange}:{card:Card;onRemove:()=>void;onExchange:(contact:string)=>void}){
 return <article className="wishlist-game-card" data-rarity={card.rarity}><div className="wishlist-card-art"><img src={card.image} alt={card.title}/><span>{card.rarity}</span><button onClick={onRemove} aria-label={`Retirer ${card.title}`}><X size={17}/></button></div><div className="wishlist-card-content"><h3>{card.title}</h3><p>{card.description}</p><div className="wishlist-card-stats"><b>ATK {card.atk.toLocaleString('fr-FR')}</b><b>DEF {card.def.toLocaleString('fr-FR')}</b></div></div><div className="direct-owners"><strong>{card.contacts.length?`${card.contacts.length} possesseur${card.contacts.length>1?'s':''}`:'Aucun contact ne la possède'}</strong>{card.contacts.map(contact=><button key={contact} onClick={()=>onExchange(contact)}><span>{contact[0]}</span>{contact}<ArrowLeftRight size={14}/></button>)}</div></article>
}

function ExchangeModal({card,contact,onClose}:{card:Card;contact:string;onClose:()=>void}){
 return <div className="exchange-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="exchange-modal" role="dialog" aria-modal="true" aria-labelledby="exchange-title"><button className="exchange-close" onClick={onClose}><X/></button><div className="exchange-icon"><ArrowLeftRight/></div><h2 id="exchange-title">Échanger avec {contact}</h2><p><b>{contact}</b> possède <b>{card.title}</b>. Vous pouvez maintenant préparer une proposition autour de cette carte.</p><div className="exchange-card-row"><img src={card.image} alt=""/><div><span>{card.rarity}</span><h3>{card.title}</h3><small>ATK {card.atk.toLocaleString('fr-FR')} · DEF {card.def.toLocaleString('fr-FR')}</small></div></div><button className="primary"><ArrowLeftRight size={18}/>Préparer l’échange</button><button className="modal-secondary" onClick={onClose}>Annuler</button></section></div>
}
