...
export default function TradeComposer({
  ...
  }, [currentUserId, draft?.contact])
  useEffect(() => {
    let active = true
    setLoadingMine(true)
    queryCollection({ page: minePage, query: mineDebounced, rarity: mineRarity })
      .then((data) => {
        if (active) {
          setMine(data.cards)
          ...
          setKnownCards(
            (current) =>
              new Map([
                ...current,
                ...data.cards.map((card) => [card.id, card] as const),
              ]),
          )
        }
      })
      .catch(() => active && setError('Impossible de charger votre collection.'))
      .finally(() => active && setLoadingMine(false))
    return () => {
      active = false
    }
  }, [minePage, mineDebounced, mineRarity])
  useEffect(() => {
    if (!friend) return
    let active = true
    setLoadingTheirs(true)
    queryFriendCollection(friend.username, {
      page: theirPage,
      query: theirDebounced,
      rarity: theirRarity,
    })
      .then((data) => {
        if (active) {
          const visible =
            theirPage === 0
              ? [
                  ...new Map(
                    [...(draft?.requestedCards ?? []), ...data.cards].map((card) => [
                      card.id,
                      card,
                    ]),
                  ).values(),
          ...
          setKnownCards(
            (current) =>
              new Map([...current, ...visible.map((card) => [card.id, card] as const)]),
          )
          setTheirsTotal((current) =>
            theirPage === 0
              ? Math.max(
                  data.total,
                  !theirDebounced && !theirRarity
                    ? (draft?.requestedCards?.length ?? 0)
                    : 0,
                )
              : current,
          )
      ...
      .catch(
        () => active && setError('Impossible de charger la collection de votre ami.'),
      )
      .finally(() => active && setLoadingTheirs(false))
    return () => {
      active = false
    }
  }, [friend, theirPage, theirDebounced, theirRarity, draft?.requestedCards])
  const shown = tab === 'mine' ? mine : theirs
  const selected = tab === 'mine' ? offered : requested
  const query = tab === 'mine' ? mineQuery : theirQuery
  const rarity = tab === 'mine' ? mineRarity : theirRarity
  const page = tab === 'mine' ? minePage : theirPage
  const total = tab === 'mine' ? mineTotal : theirsTotal
  const loading = tab === 'mine' ? loadingMine : loadingTheirs
  const selectedMineCards = useMemo(
    () =>
      offered
        .map((id) => knownCards.get(id))
        .filter((card): card is Card => Boolean(card)),
    [offered, knownCards],
  )
  const selectedTheirCards = useMemo(
    () =>
      requested
        .map((id) => knownCards.get(id))
        .filter((card): card is Card => Boolean(card)),
    [requested, knownCards],
  )
  const setQuery = (value: string) =>
    tab === 'mine' ? setMineQuery(value) : setTheirQuery(value)
  const setRarity = (value: Rarity | undefined) =>
    tab === 'mine' ? setMineRarity(value) : setTheirRarity(value)
  const setPage = (value: number) =>
    tab === 'mine' ? setMinePage(value) : setTheirPage(value)
  ...
  const send = async () => {
    if (!friend) return
    if (!offered.length && !requested.length && !credits && !requestedCredits) {
      setError('Sélectionnez au moins une carte ou des wikibidous.')
      return
    }
    if (offered.length + requested.length > 100) {
      setError('Un échange ne peut pas inclure plus de 100 cartes.')
      return
    }
    if (!confirming) {
      setConfirming(true)
  ...
  if (!friend)
    return (
      <div className="modal-backdrop">
        <section className="friend-picker" role="dialog" aria-modal="true">
  ...
  return (
    <div
      className="modal-backdrop trade-backdrop"
      onMouseDown={(event) => {
    ...
    >
      ...
      >
        <header>
          <h2 id="trade-title">
            Échanger avec <span>{friend.username}</span>
          </h2>
        ...
        <div className="trade-summary">
          <b>
            Moi : {offered.length} carte{offered.length !== 1 ? 's' : ''}
            {credits ? ` · ${credits} WB` : ''}
          </b>
          <ArrowLeftRight />
          <b>
            {friend.username} : {requested.length} carte
            {requested.length !== 1 ? 's' : ''}
            {requestedCredits ? ` · ${requestedCredits} WB` : ''}
          </b>
        </div>
        <nav>
          <button
          ...
          <button
            className={tab === 'theirs' ? 'active' : ''}
            onClick={() => setTab('theirs')}
          >
            Cartes de {friend.username}
          </button>
        </nav>
        {selectedMineCards.length || selectedTheirCards.length ? (
          <div className="trade-selected">
            {selectedMineCards.length ? (
              <section>
                <strong>Je propose</strong>
                {selectedMineCards.map((card) => (
                  <button
                    key={card.id}
                    onClick={() =>
                      setOffered((items) => items.filter((id) => id !== card.id))
                    }
                  >
            ...
            {selectedTheirCards.length ? (
              <section>
                <strong>Je demande</strong>
                {selectedTheirCards.map((card) => (
                  <button
                    key={card.id}
                    onClick={() =>
                      setRequested((items) => items.filter((id) => id !== card.id))
                    }
                  >
                    <GameCard card={card} compact />
                    <X />
        ...
        <div className="trade-tools">
          ...
          <select
            ...
            value={rarity ?? ''}
            onChange={(event) =>
              setRarity((event.target.value || undefined) as Rarity | undefined)
            }
          >
          ...
          <div className="trade-credits">
            <label>
              ...
              <input
                ...
                value={credits}
                onChange={(e) =>
                  setCredits(Math.min(10000, Math.max(0, Number(e.target.value))))
                }
              />{' '}
              WB
            </label>
        ...
        <div className="trade-card-area">
          ...
          {!loading && !shown.length ? (
            <div className="trade-no-card">Aucune carte trouvée.</div>
          ) : null}
          <div className="trade-pagination">
            ...
            <span>
              Page {page + 1} sur {Math.max(1, Math.ceil(total / 50))}
            </span>
            <button
