...
const withKey = (list:Wishlist):StoredWishlist => ({ ...list, key:`${list.ownerId}:${list.id}` })
...
export async function getWishlists(ownerId:string):Promise<Wishlist[]> {
  ...
  return new Promise((resolve,reject) => {
    const request = db.transaction(STORE).objectStore(STORE).index('ownerId').getAll(ownerId)
    request.onsuccess = () => resolve((request.result as StoredWishlist[]).map(({key:_,...list})=>list).sort((a,b)=>b.updatedAt-a.updatedAt))
    request.onerror = () => reject(request.error)
  })
}

export async function putWishlist(list:Wishlist):Promise<void> {
  if(BACKEND_URL){await backendRequest('/wishlists',{method:'PUT',body:JSON.stringify(list)});return}
  const db = await openDb()
  return new Promise((resolve,reject) => {
    const request = db.transaction(STORE,'readwrite').objectStore(STORE).put(withKey(list))
...
export async function deleteWishlist(ownerId:string,id:string):Promise<void> {
  if(BACKEND_URL){await backendRequest(`/wishlists/${encodeURIComponent(id)}`,{method:'DELETE'});return}
  const db = await openDb()
  return new Promise((resolve,reject) => {
    const request = db.transaction(STORE,'readwrite').objectStore(STORE).delete(`${ownerId}:${id}`)
    request.onsuccess = () => resolve()
    request.onerror = () => reject(request.error)
  })
}
