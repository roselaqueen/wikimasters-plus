...
export default function GameCard({card,wanted=false,onClick,onWant,compact=false,removeAction=false}:{card:Card;wanted?:boolean;onClick?:()=>void;onWant?:()=>void;compact?:boolean;removeAction?:boolean}){
 return <article className={`game-card rarity-${card.rarity.toLowerCase()} ${compact?'compact':''}`} onClick={onClick} onKeyDown={event=>{if(onClick&&(event.key==='Enter'||event.key===' '))onClick()}} tabIndex={onClick?0:undefined}>
  ...
  <div className="game-art"><img src={card.image} alt={card.title} loading="lazy"/><span>{card.rarity}</span>{onWant?<button className={removeAction?'remove-card':wanted?'wanted':''} onClick={event=>{event.stopPropagation();onWant()}} aria-label={removeAction?`Retirer ${card.title}`:wanted?'Retirer des souhaits':'Ajouter aux souhaits'}>{removeAction?<X size={16}/>:<Heart size={15} fill={wanted?'currentColor':'none'}/>}</button>:<Star size={17}/>}</div>
