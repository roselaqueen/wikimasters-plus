...
export type Wishlist = { id:string; ownerId:string; name:string; cardIds:string[]; createdAt:number; updatedAt:number; sourceOwnerId?:string; sourceListId?:string; sourceUsername?:string; readOnly?:boolean }
...
export type Trade = { id:string; status:'pending'|'accepted'|'declined'|'cancelled'|'countered'|'expired'; initiator_id:string; recipient_id:string; initiator?:{username:string}; recipient?:{username:string}; initiator_wikibidous:number; recipient_wikibidous:number; items:TradeItem[]; created_at:string }
